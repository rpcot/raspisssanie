...
const { apiPort, apiHost, defaultBellsPath, siteUrlQueryTemplate } = require('../config');
...
async function getAllWeekDates() {
    ...
    for (const week of weeks) {
        ...
        data.unshift(
            { number: week.number, date, url: `${siteUrlQueryTemplate}${week.number}` }
        );
...
app.get('/', (req, res) => {
    res.status(200).json({ ok: true, status: 200, message: 'OK' });
});
